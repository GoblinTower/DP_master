function plot_supply_mpc_simple_waypoint(t, x, x_lin, K, u, waypoints)
    ...
    %%%%%%%%%%%%
    hold on
    plot(x(2,:), x(1,:))                                              % "Real" process
    plot(x_lin(2,:), x_lin(1,:))                                      % Linear model
    plot(x(2,1), x(1,1), 'go','MarkerSize', 10);                      % Start position
    plot(x(2,end), x(1,end), 'ro','MarkerSize', 10);                  % End position
    plot(waypoints(2,:), waypoints(1,:), 'bx', 'MarkerSize', 10);     % Waypoints
    grid();
    title('Path in x-y plane');
    xlabel('East [m]');
    ylabel('North [m]');
    legend({'Path', 'Path lin', 'Start position', "End position", "Waypoints"}, 'Location', 'Best');
