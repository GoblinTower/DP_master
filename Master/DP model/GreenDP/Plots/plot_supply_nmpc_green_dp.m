function plot_supply_nmpc_green_dp(t, x, x_est, K, u, wind_abs, wind_beta, wind_force, current_force, wave_force, setpoint, save_plots, folder, file_prefix, radius, center)
    ...
    xlim([center(1)-radius-2, center(1)+radius+2]);
    ylim([center(2)-radius-2, center(2)+radius+2]);
    xlabel('East [m]');
    ylabel('North [m]');
    legend({'Path', 'Green DP border', 'Start position', 'End position'}, 'Location', 'Best');
    ...
    if (save_plots)
        ...
        save_plot(f6, file_prefix + "wave", folder);
    end
