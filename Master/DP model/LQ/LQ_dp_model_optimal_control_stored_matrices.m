...
% This model stores the state feedback matrices beforehand instead of 
% recalculating these at every timestep.
...
if (exist('external_scenario', 'var'))
    ...
    if (run_stored_matrices)
        run 'Scenarios\Prestoring_state_feedback_matrix\dp_model_scenario_LQ_control_stored_matrices_with_disturbance.m';
        workspace_file_name = strcat('Prestored\prestored_matrices_', num2str(mc_iteration));
    else
        run 'Scenarios\Prestoring_state_feedback_matrix\dp_model_scenario_LQ_control_no_storing_with_disturbance.m';
        workspace_file_name = strcat('Prestored\continuously_calculated_matrices_', num2str(mc_iteration));
    end
...
% Get prestored matrices
pre_mat = load('Scenarios\Prestoring_state_feedback_matrix\Prestored_matrices\prestored_matrices.mat');
...
for i=1:N
    ...
    if (run_using_stored_matrices)
        % Get LQ gain G1 and G2 from stored matrices
        key = mod(round(rad2deg(psi),1),360);
        key = num2str(key,'%.1f');
        
        G1 = cell2mat(pre_mat.G1_dict(key));
        G2 = cell2mat(pre_mat.G2_dict(key));
    else 
    ...
    if (run_kalman_filter)
        ...
        if (run_using_stored_matrices)
            % Get model matrices and Kalman gain from stored matrices
            A_lin = cell2mat(pre_mat.Akal(key));
            B_lin = cell2mat(pre_mat.Bkal(key));
            F_lin = cell2mat(pre_mat.Fkal(key));
            C_lin = cell2mat(pre_mat.Ckal(key));
            K = cell2mat(pre_mat.Kkal(key));
        else
