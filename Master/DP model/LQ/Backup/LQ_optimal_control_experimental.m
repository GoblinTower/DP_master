...
run 'Scenarios\supply_scenario_LQ_control_experimental';
...
for i=1:N
    ...
    if true % (run_kalman_filter)
        ...
        subm = [zeros(3); eye(3)];
        At = [A_lin, subm; zeros(3,6), eye(3)];                    % Augmented state transition matrix
        Bt = [B_lin; zeros(3,r_dim)];                              % Augmented input matrix
        Gt = [zeros(n_dim,3); eye(3)];                             % Augmented process noise matrix
        Ct = [C_lin, zeros(m_dim, 3)];                             % Augmented output matrix

        V = [eye(3)];
        [K,~,~,~] = dlqe(At, Gt, Ct, V, eye(3));
        %[K,~,~,~] = dlqe(A_lin, eye(6), C_lin, eye(6), eye(3));
