...
dt = 0.2;           % Timestep used in integration
...
K_force = diag([propeller_port_force_coefficient, propeller_starboard_force_coefficient, ...
    azimuth_thruster_force_coefficient]);
...
alpha_min = [deg2rad(-180)];
alpha_max = [deg2rad(180)];

% maximum allowed alpha change between timesteps
alpha_diff_min = [deg2rad(-360)];
alpha_diff_max = [deg2rad(360)];
...
epsilon = 1e-3; % Avoid divding by zero  
