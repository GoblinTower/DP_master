...
run 'Scenarios\supply_scenario_non_linear_mpc_control_with_external_force';
...
for i=1:N
    ...
    switch (integration_method)
        ...
        case (IntegrationMethod.Runge_Kutta_Fourth_Order)
            ...
            [~, x] = runge_kutta_4(@(t, x) supply_model(t, x, u, M, D, wind_forces(:,i), wave_forces(:,i), current_forces(:,i)), t, x, dt);
    end
    ...
    % Print unknown force estimate
    disp(['b(1): ', num2str(x_est(7))]);
    disp(['b(2): ', num2str(x_est(8))]);
    disp(['b(3): ', num2str(x_est(9)), newline]);
...
% Plot data
plot_supply_nmpc_with_disturbance(t_array, x_array, x_est_array, K_array, u_array, setpoint, ...
    wind_forces, wave_forces, current_forces, true);
