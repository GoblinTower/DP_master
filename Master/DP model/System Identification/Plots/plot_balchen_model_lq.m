function plot_dp_model_lq(t, x, x_est, K, u, wind_abs, wind_beta, wind_force, current_velocity, wave_force, setpoint, save_plots, folder, file_prefix)
    ...
    length_time = size(current_velocity,2);
    %%%%%%%%%%%%%%%
    %%% Current %%%
    %%%%%%%%%%%%%%%
    subplot(1,2,1);
    hold on;
    plot(t(1:length_time), current_velocity(1,1:length_time));
    % plot(t(1:length_time), x_est(10,1:length_time));
    grid();
    title('Current velocity in surge');
    xlabel('t [s]');
    ylabel('Vel in surge [m/s]');
    legend({'Current velocity'}, 'Location', 'Best');
    hold off

    subplot(1,2,2);
    hold on;
    plot(t(1:length_time), current_velocity(2,1:length_time));
    % plot(t(1:length_time), x_est(11,1:length_time));
    grid();
    title('Current velocity in sway');
    xlabel('t [s]');
    ylabel('Vel in sway [m/s]')
    legend({'Current velocity'}, 'Location', 'Best');
    hold off
