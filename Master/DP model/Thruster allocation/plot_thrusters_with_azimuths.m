function fig = plot_thrusters_with_azimuths(ws, time_array, font_size, length, breadth, scale_forces, scale_surge, scale_sway, scale_momentum, azi)
    ...
    for i=1:number_of_time_samples
        ...
        angle_number = 1;
        for thr_nr=1:number_of_thrusters

            % Get thruster angles
            
            % Check if azimuth
            is_azimuth = any(thr_nr == azi);
            if (is_azimuth)
                thruster_angles = ws.angle_array(angle_number, time);
                rad2deg(thruster_angles)
                angle_number = angle_number + 1;
            else
                thruster_angles = ws.thruster_angles(thr_nr);
            end
        
            % Get thruster position in BODY coordinate system
            x_pos = ws.thruster_positions(1, thr_nr);
            y_pos = ws.thruster_positions(2, thr_nr);
