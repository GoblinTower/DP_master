...
for i=1:N
    ...
    if (run_kalman_filter)
        ...
        u_sol = fmincon(@(u) non_linear_objective_du_function(u, tau, ref, M, D, P, Q, x_est, u0(:,1), horizon_length, dt, 1), u0, [], [], [], [], [], [], [], options);
    else
        % Use real state (assumed known, perfect information)
        % b is unknown, and hence set to zero
        x_extended = [x; 0; 0; 0];
        u_sol = fmincon(@(u) non_linear_objective_du_function(u, tau, ref, M, D, P, Q, x_extended, u0(:,1), horizon_length, dt, 1), u0, [], [], [], [], [], [], [], options);
    end
